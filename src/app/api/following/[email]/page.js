    ...
    return (
        <>
            ...
            <div className="absolute inset-0 -z-10 h-full w-full items-center px-5 py-24 [background:radial-gradient(125%_125%_at_50%_10%,#000_40%,#63e_100%)] flex flex-col justify-baseline text-white">
                <div className='mt-20 '>
                    <Link href={targetEmail == session?.user?.email.replace(/@/,'%40') ? '/dashboard' : `/api/person/${targetEmail}`} type="button" className="text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-full text-sm px-5 py-2.5 me-2 mb-2 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700">GO BACK</Link>
                    <div className='font-semibold mb-3 min-w-150 text-2xl text-center'>Following</div>
                    <div className='space-y-2'>
                        {targetUser.following.length > 0 ? (
                            targetUser.following.map((follower, index) => (
                                <div
                                    key={index}
